...
function CallToActionModal({setShowCallToAction}) {
    return (
        <div className = "bg-black bg-opacity-50 absolute inset-0 flex justify-center items-center font-DM ">
            ...
            <IoClose onClick = {()=>setShowCallToAction(false)} size = {20} color = "white" className = "cursor-pointer absolute -right-10 -top-10"/>
                ...
                <h1 style = {{color:'#0D0F44'}} className = "text-5xl md:text-6xl mb-10">Thank You!</h1>
                <p className = "mb-20 text-2xl md:text-3xl">Welcome to our community, we are delighted to have you <br/> with us. Surprise your partner with a <span className = "font-bold">Candid Virtual Date!</span></p>
                <button className = "bg-pink px-6 py-3 md:px-12 rounded-lg text-white text-lg md:text-2xl">Book for free</button>
                <p style = {{transform:'translateX(-50%)'}} className = "text-xl md:text-2xl font-bold absolute bottom-7 left-1/2">Why don’t you follow us on instagram as well?</p>
                </div>
